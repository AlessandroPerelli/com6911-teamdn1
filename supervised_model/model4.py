...
# similar f1 scores
# textcnn took a lot longer
#2025-04-30 23:01:38,181 INFO Test micro-F1: 0.6243
#2025-04-30 23:01:38,181 INFO Test macro-F1: 0.6056

#bilstm
#2025-04-30 23:06:42,363 INFO Test micro-F1: 0.6390
#2025-04-30 23:06:42,363 INFO Test macro-F1: 0.6249

#cnn_rnn
#2025-05-01 11:52:01,000 INFO Test micro-F1: 0.6954
#2025-05-01 11:52:01,000 INFO Test macro-F1: 0.6837
...
class HAN(nn.Module):
    def __init__(self,vocab_size,emb_dim,num_classes,word_hidden=64,sent_hidden=64,dropout=0.5):
        super().__init__()
        self.embedding=nn.Embedding.from_pretrained(torch.FloatTensor(embedding_matrix),freeze=False,padding_idx=word2idx['<PAD>'])
        self.word_rnn=nn.GRU(emb_dim,word_hidden,bidirectional=True,batch_first=True)
        self.word_attn=nn.Linear(word_hidden*2,word_hidden*2)
        self.sent_rnn=nn.GRU(word_hidden*2,sent_hidden,bidirectional=True,batch_first=True)
        self.sent_attn=nn.Linear(sent_hidden*2,sent_hidden*2)
        self.dropout=nn.Dropout(dropout)
        self.fc=nn.Linear(sent_hidden*2,num_classes)
    def forward(self,x):
        if x.dim()==2: x=x.unsqueeze(1)
        B,S,L=x.size()
        x=x.view(B*S,L)
        emb=self.embedding(x)
        w_out,_=self.word_rnn(emb)
        w_score=torch.tanh(self.word_attn(w_out))
        w_weights=torch.softmax(w_score.sum(dim=2),dim=1).unsqueeze(2)
        sent_emb=(w_out*w_weights).sum(dim=1).view(B,S,-1)
        s_out,_=self.sent_rnn(sent_emb)
        s_score=torch.tanh(self.sent_attn(s_out))
        s_weights=torch.softmax(s_score.sum(dim=2),dim=1).unsqueeze(2)
        doc_emb=(s_out*s_weights).sum(dim=1)
        return torch.sigmoid(self.fc(self.dropout(doc_emb)))
